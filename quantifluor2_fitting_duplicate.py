...
def parse_quant_file(filepath:str,matrix_barcode:str):
    ...
    rows = list("ABCDEFGH")
    source_columns = range(1, 6)

    well_map = {}

    for source_column in source_columns:
        quant_column_1 = source_column * 2 + 1
        quant_column_2 = source_column * 2 + 2

        for row in rows:

            if source_column < 10:
                original_well = f"{row}0{source_column}"

            else:
                original_well = f"{row}{source_column}"

            well_map[f"{row}{quant_column_1}"] = original_well
            well_map[f"{row}{quant_column_2}"] = original_well
    ...
    split_id=df["sample_id"].str.split("_",n=1, expand=True)

    df["sample_type_number"]=split_id[0].str[:2]+split_id[1].astype(str)
    df["sample_replicate"]=split_id[0].str[2:].astype(int)

   
    #Now we pivot to wide format
  
    pivot_df=df.pivot(

        index=["sample_type_number"],
        columns="sample_replicate",
        values=["well", "raw_rfu","original_matrix_well"]
    )

    #Now lets make sure that all of the original matrix wells for each sample replicate match. If not, there is a problem.

    replicate_1 = pivot_df[("original_matrix_well", 1)]
    replicate_2 = pivot_df[("original_matrix_well", 2)]

    wells_match = (
        replicate_1.eq(replicate_2)
        | (replicate_1.isna() & replicate_2.isna())
    )

    if not wells_match.all():
        mismatches = pivot_df.loc[
            ~wells_match,
            [
                ("original_matrix_well", 1),
                ("original_matrix_well", 2),
            ],
        ]

        raise ValueError(
            "Original matrix wells do not match between replicates:\n"
            f"{mismatches}"
        )
    ...
    matrix_well_list=pivot_df[("original_matrix_well", 1)].dropna().unique()
    ...
    pivot_df["tube_barcode"]=pivot_df[("original_matrix_well",1)].map(matrix_well_map)
...
def determine_concentration_status(row):
    ...
    replicate_1 = row[("interpolated_mass (ng)", 1)]
    replicate_2 = row[("interpolated_mass (ng)", 2)]

    reasons = []

    if pd.isna(replicate_1):
        reasons.append("replicate 1 missing")
    elif replicate_1 == "<bottom":
        reasons.append("replicate 1 below fitted bottom")
    elif replicate_1 == ">top":
        reasons.append("replicate 1 above fitted top")

    if pd.isna(replicate_2):
        reasons.append("replicate 2 missing")
    elif replicate_2 == "<bottom":
        reasons.append("replicate 2 below fitted bottom")
    elif replicate_2 == ">top":
        reasons.append("replicate 2 above fitted top")


    if reasons:
        return "; ".join(reasons)

    final_concentration = row[
        ("final_concentration (ng/ul)", "mean")
    ]

    if pd.isna(final_concentration):
        return "final concentration could not be calculated"
...
def interpolate_raw_data(data, sample_type:str):
    ...
    #now we grab the mean RFU for all the samples
    data[("raw_rfu","mean")]=(data[("raw_rfu",1)]+data[("raw_rfu",2)])/2


    #For DNA, we use all 8 points to fit the data. For RNA, there is not a difference between std7 and the blank, so we drop std7. This means also that the detection limit for RNA is higher
    ...
    data["std_mass (ng)"]=data.index.map(x_std_mapping)
    ...
    x=standards_df["std_mass (ng)"]
    y=standards_df[("raw_rfu","mean")]
    ...
    data[("interpolated_mass (ng)", 1)] = data[("raw_rfu", 1)].map( 
    ...
    data[("interpolated_mass (ng)", 2)] = data[("raw_rfu", 2)].map( 
        lambda y: four_pl_interpolate_concentration( 
            y,
            bottom_fit, 
            top_fit, 
    ...
    replicate_masses = data["interpolated_mass (ng)"][[1, 2]]

    # Temporary numeric copy for calculations.
    # "<bottom" and ">top" become NaN in this copy only.
    numeric_replicate_masses = replicate_masses.apply(
        pd.to_numeric,
        errors="coerce"
    )

    both_replicates = (
        numeric_replicate_masses[1].notna()
        & numeric_replicate_masses[2].notna()
    )

    data[("interpolated_mass (ng)", "mean")] = (
        numeric_replicate_masses
        .mean(axis=1)
        .where(both_replicates)
    )

    data[("interpolated_mass (ng)", "stdev")] = (
        numeric_replicate_masses
        .std(axis=1, ddof=1)
        .where(both_replicates)
    )
    ...
    both_replicates_0=numeric_replicate_masses.eq(0).all(axis=1)
    ...
    data[("interpolated_mass (ng)", "CV")]=CV.mask(both_replicates_0,0)


    print(data.columns.to_flat_index())

    #THESE CORRECTION FACTORS NEED TO BE CALCULATED AND UPDATED!!!!!!
        #THESE CORRECTION FACTORS NEED TO BE CALCULATED AND UPDATED!!!!!!
    ...
    data[("final_concentration (ng/ul)", "mean")] = (
        data[("interpolated_mass (ng)", "mean")]
        * sample_dilution_correction[sample_type]
    )

    data[("final_concentration (ng/ul)", "status")] = data.apply(
        determine_concentration_status,
        axis=1,
    )

    #I like the multi-index for working with, but it makes dict exports challenging. Here we flatten the multicolumn index for others to use the output of this function.
    data.columns = [
    ...
    data=data.reset_index(names="sample_description")

    rep_1_df=data[[
            "sample_description",
            "well_1", 
            "raw_rfu_1",         
            "original_matrix_well_1",
            "tube_barcode",
            "raw_rfu_mean",
            "std_mass_ng",
            "interpolated_mass_ng_1",
            "interpolated_mass_ng_mean",
            "interpolated_mass_ng_stdev",
            "interpolated_mass_ng_cv",
            "final_concentration_ng/ul_mean",
            "final_concentration_ng/ul_status"
    ]]

    rep_1_df=rep_1_df.rename(columns={
            "well_1":"well", 
            "raw_rfu_1":"raw_rfu",         
            "original_matrix_well_1":"original_matrix_well",
            "interpolated_mass_ng_1":"interpolated_mass_ng",
            "final_concentration_ng/ul_mean":"concentration",
            "final_concentration_ng/ul_status":"concentration_status"
    })
    ...
    rep_2_df=data[[
            ...
            "well_2", 
            "raw_rfu_2",         
            "original_matrix_well_2",
            "tube_barcode",
            "raw_rfu_mean",
            "std_mass_ng",
            "interpolated_mass_ng_2",
            "interpolated_mass_ng_mean",
            "interpolated_mass_ng_stdev",
            "interpolated_mass_ng_cv",
            "final_concentration_ng/ul_mean",
    ...
    rep_2_df=rep_2_df.rename(columns={
        "well_2":"well", 
        "raw_rfu_2":"raw_rfu",         
        "original_matrix_well_2":"original_matrix_well",
        "interpolated_mass_ng_2":"interpolated_mass_ng",
        "final_concentration_ng/ul_mean":"concentration",
        "final_concentration_ng/ul_status":"concentration_status"
    })
    ...
    data_long=pd.concat([rep_1_df,rep_2_df])

    data_long["well"] = (
    data_long["well"].str[0]
    + data_long["well"].str[1:].str.zfill(2)
    )

    data_long=data_long.set_index("well")
    ...
    results["placeholder_standard_qc_1"]="placeholder_qc_1"
    results["placeholder_standard_qc_2"]="placeholder_qc_2"
